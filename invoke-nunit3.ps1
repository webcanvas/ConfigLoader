...
function Invoke-NUnitTests {
  ...
  if($assemblies.Count -gt 0) {
    ...
    if ($run_dotcover -eq $true) {
      ...
      exec { & $(Get-dotCover) cover /TargetExecutable="$nunit" /TargetArguments="--work=`"`"$test_results_dir`"`" --result=`"`"NUnit.xml;format=AppVeyor`"`" --framework=`"`"net-$framework_version`"`" --noh `"`"$($assemblies -join '`"`" `"`"')`"`"" /Output="$(Join-Path $test_results_dir 'NUnit.dotCover.Snapshot.dcvr')" /Scope="$scope" /Filters="`"$script:dotcover_filters`"" /AttributeFilters="`"$script:dotcover_attribute_filters`"" /ReturnTargetExitCode }
    } elseif ($run_opencover -eq $true) {
      exec { & $(Get-OpenCover) "-target:$nunit" "-targetargs:--work=`"`"$test_results_dir`"`" --result=`"`"NUnit.xml;format=AppVeyor`"`" --framework=`"`"net-$framework_version`"`" --noh `"`"$($assemblies -join '`"`" `"`"')`"`"" "-output:$(Join-Path $test_results_dir $opencover_result_name)" -mergeoutput "-filter:$script:opencover_filters" "-excludebyattribute:$script:opencover_excluded_attributes" "-register:Path$opencover_platform" -returntargetcode }
    } else {
      exec { & $nunit "$assemblies" --work="$test_results_dir" --result="NUnit.xml;format=AppVeyor" --framework="net-$framework_version" --noh }
    }
  }
}
